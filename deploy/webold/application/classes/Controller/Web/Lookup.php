...
class Controller_Web_Lookup extends Template_Web_Core {
	...
	public function action_npc() {
		...
		$rawItems = DB::select('items.era', 'items.icon', 'npc_id', 'npc.name npc_name', 'items.name', 'item_id', 'zone_id')->from('zone_drops')->join('items')->on('items.id', '=', 'item_id')->join('npc_types npc')->on('npc.id', '=', 'npc_id')->where('zone_id', '=', $id)->where('npc_id', '=', $sort)->as_object()->execute();
		if (count($rawItems) == 0) {
			return;
		}
