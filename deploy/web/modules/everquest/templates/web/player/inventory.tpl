    ...
    <div class="row">
      <div class="col-md-12">
        <ul class="breadcrumb">
          ...
          <li><a href="/player/search">Search</a> - <a href="/player/inventory/{$character->id}">{$character->name}</a></li>
					...
					<div class="col-md-12 statistics-border">
						<div class="item-stats col-md-4" style="padding: 0px 5px">
							<div class="panel panel-default">
								...
								<div class="panel-body">Name: {if !empty($character->title)}{$character->title} {/if}{$character->name} {if !empty($character->last_name)}{$character->last_name} {/if}{if !empty($character->suffix)}{$character->suffix} {/if}<br>
								Level: {$character->level}<br>
								Race: {$character->race}<br>
								Class: {$character->class_name}<br>
								Deity: {$character->deity}<br>
								Zone: {$character->zone_id}<br>
								{if $character->zone_instance != 0}Instance: {$character->zone_instance}<br>{/if}
								X: {$character->x|number_format:2:".":","}<br>
								Y: {$character->y|number_format:2:".":","}<br>
								Z: {$character->z|number_format:2:".":","}<br>
								Heading: {$character->heading|number_format:2:".":","}</div>
							</div>
						</div>
						<div class="item-stats col-md-4" style="padding: 0px 5px">
							<div class="panel panel-default">
								<div class="panel-heading">Main Statistics</div>
								<div class="panel-body">Health: {$character->cur_hp|number_format:0:".":","}<br>
								{if $character->mana != 0}Mana: {$character->mana|number_format:0:".":","}<br>{/if}
								Endurance: {$character->endurance|number_format:0:".":","}<br></div>
							</div>
						</div>
						<div class="item-stats col-md-4" style="padding: 0px 5px">
							<div class="panel panel-default">
								<div class="panel-heading">Combat Statistics</div>
								<div class="panel-body">Agility: {$character->agi|number_format:0:".":","}<br>
								Charisma: {$character->cha|number_format:0:".":","}<br>
								Dexterity: {$character->dex|number_format:0:".":","}<br>
								Intelligence: {$character->int|number_format:0:".":","}<br>
								Stamina: {$character->sta|number_format:0:".":","}<br>
								Strength: {$character->str|number_format:0:".":","}<br>
								Wisdom: {$character->wis|number_format:0:".":","}</div>
							</div>
						</div>
					</div>
				</div>
          ...
          <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="input-group">
                    Quantity: <div id="detail-quantity-slider"></div><span id="detail-quantity">1</span>
                </div>
                <div class="input-group">
