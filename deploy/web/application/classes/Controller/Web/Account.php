...
class Controller_Web_Account extends Template_Web_Core {
	...
	public function before() {
		...
		$this->template->site->description = "Account Section";
	}
	...
	public function action_changepassword() {
		...
		$account = DB::select()->from('tblLoginServerAccounts')->where('AccountSession', '=', $session)->as_object()->execute()->current();
		if (empty($account)) {
			$this->template->errorMessage = "Invalid Session Provided. Go in game and type #builds to create a new session.";
			return;
		}
		...
		if ($account->AccountPassword != "") {
			$this->template->errorMessage = "Session no longer valid, password has been set.";
		}
