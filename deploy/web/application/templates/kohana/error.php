...
$request = Request::$current;
    //Get config data
    $config = Kohana::$config->load('core');

$error_id = uniqid('error');

if (PHP_SAPI != 'cli') { //Not command line request

	// Unique error identifier
	$ip = $_SERVER['REMOTE_ADDR'];
	$isAllowed = false;
	foreach ($config['whitelist'] as $whitelist) {
		if ($whitelist == $ip) $isAllowed = true;
	}

	if (!$isAllowed) {

		//This is a 404 without being a developer IP
		if (!empty($request)) {
			//We have request data
			$directory = strtolower($request->directory());
			$controller = strtolower($request->controller());
			$action = strtolower($request->action());
			$uri = $request->uri();

			if (strpos($uri, "rest") === 1) { //Not a rest call

				$rest = new stdClass();
				$rest->Status = 0;
				$rest->Message = "Invalid Rest Call Received";
				$rest->Data = "$url";
				die(jsone_encode($rest));
			...
			/*	if ($directory == "admin") {
					die("Rawr");
					$template = View::factory("admin/_template");
					//auto load _template_top_js if exists and none previously set in action

					
					$template->content = View::factory("admin/error/404");
				} else if ($directory == "member") {

					$template = View::factory("member/_template");
					$template->content = View::factory("member/error/404");
				} else {
					*/
		...
		$template = View::factory("$directory/_template");
		$template->site = (object)$config['site'];
		$template->breadcrumbs = ['<a href="/">Home</a>', '404'];
		$template->headline = "404";
		if (empty($template->jsTopTemplate) && Kohana::find_file("templates", "$directory/_template_top_js", "tpl")) {
			$template->jsTopTemplate = View::factory("$directory/_template_top_js");
		}

		//auto load _template_bot_js if exists and none previously set in action
		if (empty($template->jsBotTemplate) && Kohana::find_file("templates", "$directory/_template_bot_js", "tpl")) {
			$template->jsBotTemplate = View::factory("$directory/_template_bot_js");
		}

		//auto load action_top_js if exists and none previously set in action
		if (empty($template->jsTopPage) && Kohana::find_file("templates", "$directory/error/404_top_js", "tpl")) {
			$template->jsTopPage = View::factory("$directory/error/404_top_js");
		}

		//auto load action_bot_js if exists and none previously set in action
		if (empty($template->jsBotPage) && Kohana::find_file("templates", "$directory/error/404_bot_js", "tpl")) {
			$template->jsBotPage = View::factory("$directory/error/404_bot_js");
		}
		$template->content = View::factory("$directory/error/404");
		header("HTTP/1.0 404 Not Found");
		die($template->render());
	}
	//This means it was allowed, so it's going to do a stack trace.
}
 ...
 */
if (PHP_SAPI == 'cli') {
	echo "$class [ $code ]: $message: (line $line on $file)\n";
	foreach (Debug::trace($trace) as $i => $step) {
		if ($step['file']) {
			$source_id = $error_id.'source'.$i;
		}
		echo "Path: ".$step['file']." on Line: ".$step['line'].", function: ".$step['function']."\n";
		if ($step['args']) {
...
/**
 * We got far enough to doa  request
 */
if (!empty($request)) {
	$directory = strtolower($request->directory());
	$controller = strtolower($request->controller());
	$action = strtolower($request->action());
		$uri = $request->uri();
		//Capture REST specific errors
		if (strpos($uri, "rest") === 0) {
		$message = "$class [ $code ]: ".htmlspecialchars( (string) $message, ENT_QUOTES, Kohana::$charset, TRUE).": (line $line on $file)";
		$rest = new stdClass();
		$rest->Status = 0;
		...
		foreach (Debug::trace($trace) as $i => $step) {
			...
			$rest->Data[$j]['path'] = Debug::path($step['file']);
			$rest->Data[$j]['line'] = $step['line'];
			$rest->Data[$j]['function'] = $step['function'];
			...
			if (isset($args_id)) {
				...
				foreach ($step['args'] as $name => $arg) {
					$rest->Data[$j][$k] = array();
					$rest->Data[$j][$k]['args'] = $name.", ".Debug::dump($arg);
					$k++;
				}
			}
			if (isset($source_id)) {
				$rest->Data[$j]['source_id'] = $source_id;
				$rest->Data[$j]['source'] = $step['source'];
			}
...
$config = Kohana::$config->load('core');('core');

/**
 * Fall back, web request
 */
