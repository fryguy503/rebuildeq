...
{	
	...
	case BARD:
		...
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Virtuoso", "Virtuoso").c_str(),
			c->CreateSayLink("#Swordsinger", "Swordsinger").c_str(),
			c->CreateSayLink("#Maestro", "Maestro").c_str()
			);
		break;
	case CLERIC:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Templar", "Templar").c_str(),
			c->CreateSayLink("#Bishop", "Bishop").c_str(),
			c->CreateSayLink("#Archon", "Archon").c_str()
			);
		break;
	case DRUID:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Preserver", "Preserver").c_str(),
			c->CreateSayLink("#Warden", "Warden").c_str(),
			c->CreateSayLink("#Hierophant", "Hierophant").c_str()
			);
		break;
	case ENCHANTER:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Illusionist", "Illusionist").c_str(),
			c->CreateSayLink("#Entrancer", "Entrancer").c_str(),
			c->CreateSayLink("#Beguiler", "Beguiler").c_str()
			);
		break;
	case MAGICIAN:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Conjurer", "Conjurer").c_str(),
			c->CreateSayLink("#Elementalist", "Elementalist").c_str(),
			c->CreateSayLink("#Geomancer", "Geomancer").c_str()
			);
		break;
	case MONK:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Master", "Master").c_str(),
			c->CreateSayLink("#Trascendant", "Trascendant").c_str(),
			c->CreateSayLink("#Ashenhand", "Ashenhand").c_str()
			);
		break;
	case NECROMANCER:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Heretic", "Heretic").c_str(),
			c->CreateSayLink("#Lich", "Lich").c_str(),
			c->CreateSayLink("#Warlock", "Warlock").c_str()
			);
		break;
	case PALADIN:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Protector", "Protector").c_str(),
			c->CreateSayLink("#Knight", "Knight").c_str(),
			c->CreateSayLink("#Crusader", "Crusader").c_str()
			);
		break;
	case RANGER:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Archer", "Archer").c_str(),
			c->CreateSayLink("#Hunter", "Hunter").c_str(),
			c->CreateSayLink("#Plainswalker", "Plainswalker").c_str()
			);
		break;
	case ROGUE:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Assassin", "Assassin").c_str(),
			c->CreateSayLink("#Deceiver", "Deceiver").c_str(),
			c->CreateSayLink("#Bandit", "Bandit").c_str()
			);
		break;
	case SHADOWKNIGHT:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Bloodreaver", "Bloodreaver").c_str(),
			c->CreateSayLink("#Defiler", "Defiler").c_str(),
			c->CreateSayLink("#Revenant", "Revenant").c_str()
			);
		break;
	case SHAMAN:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Prophet", "Prophet").c_str(),
			c->CreateSayLink("#Oracle", "Oracle").c_str(),
			c->CreateSayLink("#Elder", "Elder").c_str()
			);
		break;
	case WARRIOR:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Gladiator", "Gladiator").c_str(),
			c->CreateSayLink("#Brawler", "Brawler").c_str(),
			c->CreateSayLink("#Champion", "Champion").c_str()
			);
		break;
	case WIZARD:
		c->Message(0, "Select a style: [%s], [%s], [%s]",
			c->CreateSayLink("#Evoker", "Evoker").c_str(),
			c->CreateSayLink("#Channeler", "Channeler").c_str(),
			c->CreateSayLink("#Sage", "Sage").c_str()
			);
		break;
	default:
		c->Message(0, "This command is not available at this time.");
		break;
	}
}
