...
int32 Mob::CheckAggroAmount(uint16 spell_id, Mob *target, bool isproc)
{
	...
	for (int o = 0; o < EFFECT_COUNT; o++) {
		switch (spells[spell_id].effectid[o]) {
			...
			case SE_MovementSpeed: {
				...
				int rank = GetBuildRank(SHADOWKNIGHT, RB_SHD_DARKNESS);
				if (rank > 0 && (
					spell_id == 344 ||
					spell_id == 355 ||
					spell_id == 452 ||
					...
					int bonus_hate = int(AggroAmount * 0.4f * rank);
					BuildEcho(StringFormat("Darkness %i increased hate generated from % u to %i.", rank, AggroAmount, AggroAmount + bonus_hate));
					AggroAmount += bonus_hate;
				}
				break;
			}
			case SE_AttackSpeed:
	...
	rank = GetBuildRank(ENCHANTER, RB_ENC_SUBTLECASTING);
