...
void Mob::MakePoweredPet(uint16 spell_id, const char* pettype, int16 petpower,
	...
	// Overwrite magician pet's npc_type here
	...
	if(spell_id == 58 || spell_id == 315 || spell_id == 316 || spell_id == 317) {
		...
		if(spell_id == 315) { // Water
			...
			npc_type->AC *= 0.8f;
			npc_type->texture = 1;
			npc_type->npc_spells_id = 17;
		}else if(spell_id == 317) { // Air
			npc_type->max_hp *= 0.69f;
			npc_type->AC *= 0.8f;
			npc_type->texture = 3;
			npc_type->npc_spells_id = 13;
		} else { // Earth
			npc_type->texture = 0;
	...
	if (this->IsClient() && CastToClient()->GetBuildRank(SHADOWKNIGHT, RB_SHD_STEADFASTSERVANT) > 0 &&
		...
		if (CastToClient()->GetBuildRank(SHADOWKNIGHT, RB_SHD_STEADFASTSERVANT) >= 5) {
			...
			if (this->CastToClient()->GetInv().GetItem(EQEmu::invslot::slotPrimary)) { //If a weapon is equipped
				
				npc_type->prim_melee_type = this->CastToClient()->GetInv().GetItem(EQEmu::invslot::slotPrimary)->GetItem()->ItemType;
			}
