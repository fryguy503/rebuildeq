...
public:
	...
	void SendEvent(pb::OpCode op, uint32 entity_id, void * buffer, size_t size);

	void OnDailyGain(int account_id, int character_id, const char * identity, int levels_gained = 0, int experience_gained = 0, int money_earned = 0);
...
protected:
	...
	void EncodeEntity(Entity *entity, pb::Entity *out);
};
